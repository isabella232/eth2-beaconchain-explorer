	...
	go func() {
		...
		for {
			...
			for slot := lastHeadSlot; slot < uint64(parsedSyncing.Data.HeadSlot); slot++ {
				blks, err := lc.GetBlocksBySlot(slot, nil) //TODO add accounts
				if err != nil {
...
func (lc *LighthouseClient) GetEpochData(epoch uint64, accounts types.Accounts) (*types.EpochData, error) {
	...
	data.ValidatorAssignmentes, err = lc.GetEpochAssignments(epoch, accounts)
	if err != nil {
	...
	for slot := epoch * utils.Config.Chain.SlotsPerEpoch; slot <= (epoch+1)*utils.Config.Chain.SlotsPerEpoch-1; slot++ {
		...
		blocks, err := lc.GetBlocksBySlot(slot, accounts)
