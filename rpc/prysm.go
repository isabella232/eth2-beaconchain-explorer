...
func NewPrysmClient(endpoint string, accounts types.Accounts) (*PrysmClient, error) {
...
func (pc *PrysmClient) GetEpochData(epoch uint64, accounts types.Accounts) (*types.EpochData, error) {
	...
	start := time.Now()
	validatorBalances, err := pc.getBalancesForEpoch(int64(epoch), pubeys)
	...
	validatorBalances1d, err := pc.getBalancesForEpoch(epoch1d, pubeys)
	...
	validatorBalances7d, err := pc.getBalancesForEpoch(epoch7d, pubeys)
	...
	validatorBalances31d, err := pc.getBalancesForEpoch(epoch31d, pubeys)
...
func (pc *PrysmClient) getBalancesForEpoch(epoch int64, pubeys [][]byte) (map[uint64]uint64, error) {
	...
	validatorBalancesRequest := &ethpb.ListValidatorBalancesRequest{PublicKeys: pubeys, PageSize: utils.Config.Indexer.Node.PageSize, PageToken: validatorBalancesResponse.NextPageToken, QueryFilter: &ethpb.ListValidatorBalancesRequest_Epoch{Epoch: uint64(epoch)}}
	if epoch == 0 {
...
func (pc *PrysmClient) parseRpcBlock(block *ethpb.BeaconBlockContainer, accounts types.Accounts) (*types.Block, error) {
	b := &types.Block{
		Status:       1,
